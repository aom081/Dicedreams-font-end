...
const Index = () => {
  ...
  async function searchUser() {
    try {
      ...
      const url = `https://dicedreams-backend-deploy-to-render.onrender.com/api-docs/users`;
  ...
  async function searchStore() {
    try {
      const token = localStorage.getItem("access_token");
      if (!token) throw new Error("No token found");
      const url = `https://dicedreams-backend-deploy-to-render.onrender.com/api-docs/store`;
      const response = await axios.get(url, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  async function getStore(id) {
    try {
      const token = localStorage.getItem("access_token");
      if (!token) throw new Error("No token found");
      const url = `https://dicedreams-backend-deploy-to-render.onrender.com/api-docs/postActivity/store/${id}`;
      const response = await axios.get(url, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  async function getUserAc(id) {
    try {
      const token = localStorage.getItem("access_token");
      if (!token) throw new Error("No token found");
      const url = `https://dicedreams-backend-deploy-to-render.onrender.com/api-docs/postGame/user/${id}`;
      const response = await axios.get(url, {
        headers: { Authorization: `Bearer ${token}` },
      });
